...
**API Base URL:** `http://localhost:8081/api/v1/executor`
...
```bash
curl -X GET 'http://localhost:8081/api/v1/executor/help'
```
...
```bash
curl -X GET 'http://localhost:8081/api/v1/executor/help?command=extract'
```
...
```bash
curl -X GET 'http://localhost:8081/api/v1/executor/snapshot'
```
...
#### Navigate to URL
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/navigate' \
...
#### Click Element
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/click' \
  -H 'Content-Type: application/json' \
  -d '{"identifier": "@e1"}'
```
**Identifier formats:**
...
#### Type Text
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/type' \
  -H 'Content-Type: application/json' \
...
#### Extract Data
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/extract' \
  -H 'Content-Type: application/json' \
  -d '{
...
#### Wait for Element
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/wait' \
  -H 'Content-Type: application/json' \
...
#### Batch Operations
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/batch' \
  -H 'Content-Type: application/json' \
  -d '{
    "operations": [
...
```bash
curl -X POST 'http://localhost:8081/api/v1/executor/press-key' \
  -H 'Content-Type: application/json' \
  -d '{"key": "Enter"}'
```
...
```bash
POST http://localhost:8081/api/v1/executor/navigate
{"url": "https://example.com/login"}
```

**Step 2:** Get page structure
```bash
GET http://localhost:8081/api/v1/executor/snapshot
```
Response:
```
Clickable Elements:
...
```bash
POST http://localhost:8081/api/v1/executor/type
...
```bash
POST http://localhost:8081/api/v1/executor/click
{"identifier": "@e1"}
```

**Step 6:** Wait for login success (optional)
```bash
POST http://localhost:8081/api/v1/executor/wait
